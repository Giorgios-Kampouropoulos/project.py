...
def csv_getter():
    ...
    """
    for i in range(5):
        file = input("input your CSV dataset path, or if it is in the same folder as the script, the CSV file name: \n")
        if file.endswith(".csv"):
            try:
                return file
    
            except FileNotFoundError:
                print("File not found. Check file name and location or the validity of the path. \n")
                continue
        else:
            print("This is not a CSV file. \n")
            if i==4:
                exit("you didn't provide me with a CSV file")
            continue
